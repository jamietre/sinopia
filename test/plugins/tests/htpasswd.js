...
const verify_password = require('../utils').verify_password
const add_user_to_htpasswd = require('../utils').add_user_to_htpasswd
...
describe('add_user_to_htpasswd', function() {
  ...
  it('should append user / newline checks', function() {
    let res = add_user_to_htpasswd('testtest', 'user', 'passwd')
    assert(res.match(/^testtest\nuser:[^:\n]+:autocreated [^\n]+\n$/))
    res = add_user_to_htpasswd('testtest\n', 'user', 'passwd')
    assert(res.match(/^testtest\nuser:[^:\n]+:autocreated [^\n]+\n$/))
    res = add_user_to_htpasswd('testtest\n\n', 'user', 'passwd')
    assert(res.match(/^testtest\n\nuser:[^:\n]+:autocreated [^\n]+\n$/))
  })
