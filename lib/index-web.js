...
module.exports = function(config, auth, storage) {
    ...
    storage.get_local(function(err, packages) {
      ...
      async.filterSeries(packages, function(pkg, cb) {
        ...
        packages.forEach((pkg)=> {
          pkg.nameUrlEncoded = encodeURIComponent(pkg.name)
        });
  ...
  app.use('/-/doc/:package/*', function(req, res) {
    const path = req.params[0] ? req.params[0] : 'index.html'
    const pkg = req.params.package
    const pkg_root = storage.local.storage(pkg)
    const doc_root = Path.resolve(pkg_root.path, 'doc')

    res.sendFile(path, { root: doc_root })
  })
