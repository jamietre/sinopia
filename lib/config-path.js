...
function create_config_file(config_path) {
  ...
  var created_config = fs.readFileSync(require.resolve('../conf/default.json'), 'utf8')
...
function get_paths() {
  ...
  if (xdg_config && folder_exists(xdg_config)) {
    try_paths.push({
      path: Path.join(xdg_config, 'sinopia', 'config.json'),
      type: 'xdg',
    })
  }
  ...
  if (process.platform === 'win32'
   ...
   && folder_exists(process.env.APPDATA)) {
    try_paths.push({
      path: Path.resolve(Path.join(process.env.APPDATA, 'sinopia', 'config.json')),
      type: 'win',
    })
  }

  try_paths.push({
    path: Path.resolve(Path.join('.', 'sinopia', 'config.json')),
    type: 'def',
  })

  // backward compatibility
  try_paths.push({
    path: Path.resolve(Path.join('.', 'config.json')),
    type: 'old',
  })
