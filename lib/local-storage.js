...
var rimraf = Utils.promisify(require('rimraf'))
//
...
function Storage(config) {
  ...
  self.get_package_async = Utils.promisify(self, 'get_package')
  self.update_package_async = Utils.promisify(self, 'update_package')
...
/**
 * Extract documentation from the tarball as identified by a node in package.json "doc.html" which
 * specifies a path relative to the project root.
 *
 * If the node is not present, look in "doc/html" anyway
 *
 * If documentation exists, it will be extracted to "doc" in the package storage folder, otherwise that folder
 * will not exist (will be deleted if there was a prior version)
 *
 * @param {string} name The package name
 * @param {function} callback Callback to invoke when finished
 */
function extract_docs(name, callback) {
  let self = this

  const storage = self.storage(name)

  this.get_package_async(name).then((result) => {
   
    const latest = Utils.latest(result)
    
    const pkg = result.versions[latest];

    const doc_path = pkg.doc && pkg.doc.html ?
      pkg.doc.html :
      'doc/html'

    const doc_root = Path.resolve(storage.path, "doc")
    
    const tarball_name = Path.basename(pkg.dist.tarball);
    const tarball_path  = Path.resolve(storage.path, tarball_name)
    
    let hasDocIndex = false;
    return rimraf(doc_root).then(()=>{
      const validPath = `package/${doc_path}`
      return decompress(tarball_path, doc_root, {
        filter: (file)=> {
          return file.path.startsWith(validPath)
        },
        map: (file)=> {
          file.path = file.path.substring(validPath.length)
          
          if (file.path === '/index.html') { hasDocIndex = true }
          return file
        }
      }).then(()=> {
          return self.update_package_async(name, (data, cb)=> {
            let my_version = data.versions[latest];
            my_version.doc = my_version.doc || {};
            my_version.doc._hasIndex = hasDocIndex;
...
Storage.prototype.extract_docs = extract_docs
...
Storage.prototype.get_package = function(name, options, callback) {
  ...
  storage.read_json(info_file, function(err, result) {
    ...
    result._storage_path = storage.path
