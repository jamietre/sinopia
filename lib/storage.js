...
Storage.prototype.extract_docs = function extract_docs(name, callback) {
  this.local.extract_docs(name, callback)
}
...
Storage.prototype.get_package = function get_package(name, options, callback) {
  ...
  self.locker.readLock(name, callback, function(unlock) {
    ...
    self.local.get_package(name, options, function(err, data) {
      ...
      let storage_path
      if (data) {
        storage_path = data._storage_path;
        delete data._storage_path
      }
      ...
      self._sync_package_with_uplinks(name, data, options, function(err, result, uplink_errors) {
        ...
        result._storage_path = storage_path
...
Storage._merge_versions = function(local, up, config) {
  ...
  for (let i in up['dist-tags']) {
    let added = Utils.tag_version(local, up['dist-tags'][i], i, config || {})
    if (i === 'latest' && added) {
