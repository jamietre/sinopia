...
function load_plugins(config, plugin_configs, params, sanity_check) {
  var plugins = Object.keys(plugin_configs || {}).map(function(p) {
    ...
    //built-in
    if (plugin == null && p.startsWith('./plugins')) {
      const plugin_path = Path.resolve(__dirname, '..', p)
      plugin = try_load(plugin_path)
    }


    if (plugin == null) {
      const plugin_path = Path.resolve(__dirname, '../plugins', p)
      plugin = try_load(plugin_path)
    }
